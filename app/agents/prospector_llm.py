...
class LeadScore(BaseModel):
    ...
    matched_criteria: Dict[str, bool] = Field(
        description="Dictionary indicating which ICP criteria were met",
        example={
            "industry_match": True,
    ...
    recommendations: str = Field(description="Recommendations for next steps based on lead evaluation")
...
class BatchLeadScore(BaseModel):
    ...
    lead_scores: List[LeadScore] = Field(
        description="List of lead scores, qualification status and reasoning in the same order as input leads",
        min_items=5,
        max_items=5
    )
